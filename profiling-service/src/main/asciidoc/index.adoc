...
[role="primary"]
.HTTP
include::{snippets}/fields-create-example/http-request.adoc[]
[role="secondary"]
.CURL
include::{snippets}/fields-create-example/curl-request.adoc[]
[role="secondary"]
.HTTPie
include::{snippets}/fields-create-example/httpie-request.adoc[]

==== Example Response
include::{snippets}/fields-create-example/http-response.adoc[]
...
A `PATCH` request is used to update a field.

==== Request Structure

include::{snippets}/field-update-example/request-fields.adoc[]

// This only applies if/when there is more than one editable field:
//To leave an attribute of a field unchanged, any of the above may be omitted from the request.

[NOTE]
It is not possible to update the `id` or `panelId` properties of a field.
